...
A working personal tool I use for my own practice — not a polished multi-user product (row-level security is currently permissive, since it's single-user). The video analysis is the part I'm most likely to keep extending — next on the list is tracking a metric like X-Factor across sessions to see real trend lines, not just a one-off snapshot per swing.
